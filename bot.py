...
ALLOCATION_EXTRA_ALIASES = {
    ...
    # Heavily allocated bottles that bottles.json does not carry yet. The tracker
    # only needs a stable display name, so alias them here instead of blocking a log.
    "ELMER T LEE": "Elmer T. Lee Single Barrel",
    "ELMER T. LEE": "Elmer T. Lee Single Barrel",
    "ETL": "Elmer T. Lee Single Barrel",
    "WSR": "Weller Special Reserve",
    "WELLER SPECIAL RESERVE": "Weller Special Reserve",
    "GREEN LABEL WELLER": "Weller Special Reserve",
    "ROCK HILL FARMS": "Rock Hill Farms",
    "RHF": "Rock Hill Farms",
    "HANCOCKS": "Hancock's President's Reserve",
    "HANCOCKS PRESIDENTS RESERVE": "Hancock's President's Reserve",
    "OLD FITZGERALD": "Old Fitzgerald Bottled-in-Bond",
    "OLD FITZ": "Old Fitzgerald Bottled-in-Bond",
    "OFBIB": "Old Fitzgerald Bottled-in-Bond",
    "VERY OLD BARTON": "Very Old Barton",
    "VOB": "Very Old Barton",
    "LOT B": "Van Winkle Special Reserve 12 Year (Lot B)",
    "VAN WINKLE LOT B": "Van Winkle Special Reserve 12 Year (Lot B)",
    "PAPPY 20": "Pappy Van Winkle 20 Year",
    "PVW20": "Pappy Van Winkle 20 Year",
    "PAPPY 23": "Pappy Van Winkle 23 Year",
    "PVW23": "Pappy Van Winkle 23 Year",
    "LITTLE BOOK": "Little Book",
    "COY HILL": "Old Forester 117 Series Coy Hill",
    "WIDOW JANE": "Widow Jane",
    "REDWOOD EMPIRE": "Redwood Empire",
    "OFBB": "Old Forester Birthday Bourbon",
    "BIRTHDAY BOURBON": "Old Forester Birthday Bourbon",
    "FRLE": "Four Roses Limited Edition Small Batch",
    "HENRY MCKENNA 10": "Henry McKenna 10 Year Bottled-in-Bond",
    "HM10": "Henry McKenna 10 Year Bottled-in-Bond",
    "PARKERS HERITAGE": "Parker's Heritage Collection",
    "PHC": "Parker's Heritage Collection",
    "MICHTERS 20": "Michter's 20 Year Bourbon",
    "M20": "Michter's 20 Year Bourbon",
    "MICHTERS 25": "Michter's 25 Year Bourbon",
}
...
def handybot_embed(bottle_name: str, creator: discord.abc.User):
    embed = discord.Embed(
        ...
        description=(
            ...
            "If you snagged it, click **I snagged it** and upload a bottle photo."
        ),
        color=discord.Color.from_str("#C9973A"),
    )
...
async def forward_handybot_photos(
    *,
    session: dict,
    guild: discord.Guild,
    ...
    admin_channel = await handybot_admin_channel(guild)
    ...
    for index, attachment in enumerate(image_attachments, start=1):
        ...
        embed.add_field(name="Snagged by", value=user.mention, inline=True)

        if source_url:
            embed.add_field(name="Source", value=f"[Jump to HandyBot]({source_url})", inline=True)
...
class HandyBotSnagModal(discord.ui.Modal, title="Confirm HandyBot Snag"):
    def __init__(self, session_id: int):
        super().__init__()
        self.session_id = session_id
        self.photo = discord.ui.FileUpload(
            custom_id=f"handy_photo:{session_id}",
            required=True,
            min_values=1,
            max_values=5,
        )
        self.note = discord.ui.TextInput(
            label="Optional note",
            placeholder="Store, time, funny detail, or leave blank.",
            required=False,
            max_length=180,
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        if not interaction.guild or not interaction.channel:
            await interaction.response.send_message("HandyBot only works in a server thread.", ephemeral=True)
            return
        ...
        image_attachments = [attachment for attachment in self.photo.values if is_image_attachment(attachment)]
...
async def maybe_send_chat_trigger_image(message: discord.Message):
    ...
    if FELLAS_PATTERN.search(content):
        ...
        filename = "koala.png"
    elif YUH_PATTERN.search(content):
        image_path = YUH_IMAGE_PATH
        filename = "yuh.png"
